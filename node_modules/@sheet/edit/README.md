...
The most effective way to test the "password to open" is to attempt to read the
file with the password.  If the password is incorrect, the reader will throw an
error with message `"Incorrect Password"`

The following script in NodeJS will

- read a normal file with no password
- create a new file with password "sheetjs"
- fail to read the file back with no password / wrong password
- successfully read the file back with the correct password

```js
/* Read normal file */
var wb = XLSX.readFile("original.xlsx");

/* Generate new file with password "sheetjs" */
XLSX.writeFile(wb, "pwd.xlsx", {password: "sheetjs"});

/* Attempt to read file with no password / incorrect password */
var good = true;
try { XLSX.readFile("pwd.xlsx"); good=false; } catch(e){}
try { XLSX.readFile("pwd.xlsx", {password: "foobar"}); good=false; } catch(e){}
if(!good) throw new Error("read failed to reject bad password");

/* Read the file with the correct password */
var newwb = XLSX.readFile("pwd.xlsx", {password: "sheetjs"});
```
...
/* Test Workbook Properties password */
var wbenc = ((wb.Workbook || {}).Protection || {}).encryption;
var wbtest = wbenc ? XLSX.utils.test_password(wbenc, testpwd) : true;

/* Test Worksheet Properties password */
var ws = wb.Sheets["Sheet1"];
var wsenc = (ws["!protect"] || {}).encryption;
var wstest = wsenc ? XLSX.utils.test_password(wsenc, testpwd) : true;

/* Set Worksheet Password */
if(!wb.Sheets.Sheet2["!protect"]) wb.Sheets.Sheet2["!protect"] = {};
wb.Sheets.Sheet2["!protect"].encryption = XLSX.utils.hash_password("SheetJS");
```
...
The delete method will not attempt to correct formulae or other deep references,
and should only be used when known to be safe.

Defined names referencing the worksheet will be removed.
